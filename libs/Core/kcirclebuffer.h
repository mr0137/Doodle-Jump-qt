...
template<class T>
class CORE_EXPORT KCircleBuffer{
public:
    KCircleBuffer(int size=6*100000) : capacity(size) , m_data(size) {}
    bool push(const T& a);
...
template<class T>
int KCircleBuffer<T>::get_capacity()
{
    return capacity;
}
template<class T>
int KCircleBuffer<T>::get_read_pos()
{
    return capacity;
}
template<class T>
int KCircleBuffer<T>::get_write_pos()
{
    return write_pos;
}
template<class T>
void KCircleBuffer<T>::ClearToSaveZone()
{
...
template<class T>
bool KCircleBuffer<T>::push(const T &a) {
    now_in_buffer++;
    m_data[write_pos] = a;
